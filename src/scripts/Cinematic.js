...
const DREAM_SCENES = [
  {
    ...
    narration: '💭 "Last night, you fell asleep on your cardboard box... and dreamed of something better."',
    floaters: ['💰','💎','🪙','✨','💛','🏆'],
  },
  {
    bg: 's2',
    ...
    narration: '💭 "You owned three yachts. One was just for storing the other two."',
    floaters: ['🌊','⚓','🐬','🦞','🍾','💸'],
  },
  {
    bg: 's3',
    ...
    narration: '💭 "Your suit cost more than a small country GDP. You wore it to buy groceries."',
    floaters: ['👑','💍','💎','🪄','✨','🎩'],
  },
  {
    bg: 's4',
    ...
    narration: '💭 "You had a personal eagle. For no reason. It was named Gerald."',
    floaters: ['🦅','📈','🏦','💼','🌍','🏅'],
  },
];
...
function buildCinematicHTML() {
  const scenes = DREAM_SCENES.map((s, i) => `
  ...
  const dots = DREAM_SCENES.map((_, i) => `<div class="prog-dot" id="dot-${i}"></div>`).join('');
...
function runDreamSequence(overlay, onComplete) {
  ...
  function showScene(idx) {
    if (currentScene >= 0) {
      document.getElementById('scene-' + currentScene)?.classList.remove('visible');
  ...
  setTimeout(() => {
    document.getElementById('dot-' + (DREAM_SCENES.length - 1))?.classList.add('done');
  }, TOTAL_DREAM_TIME - 200);

  setTimeout(() => triggerExplosion(overlay, onComplete), TOTAL_DREAM_TIME + 400);
}
...
function triggerExplosion(overlay, onComplete) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { canContinue = true; }, 1200);
